...
**▶ https://haruverse.haru.company** — 맥미니 셀프호스팅으로 직접 운영 중입니다.
(HTTP는 국내 회선에서 80 포트가 막혀 있어 `https://` 로 접속해야 합니다)
...
맥미니(M4, macOS)에서 직접 운영합니다. 클라우드 비용 없이 **인프라까지 직접 다뤄본 것**을 보이려는 목적도 있습니다.

```
인터넷 ──443──▶ nginx (리버스 프록시)
                   └──▶ 127.0.0.1:5300  프론트(nginx+정적)
                            └── /api ──▶ backend:8080 ──┬──▶ Elasticsearch
                                                        └──▶ MariaDB
```

`docker-compose.macmini.yml` 이 로컬 구성과 다른 점 세 가지입니다.

**① MariaDB를 새로 띄우지 않습니다.**
맥미니에 다른 프로젝트가 쓰는 MariaDB가 이미 돕니다. 컨테이너를 또 띄우면 3306이 충돌하고 메모리도 두 배로 씁니다. `haruverse` DB만 분리해 함께 씁니다.
컨테이너가 아니라 호스트의 것이므로 서비스명이 아니라 `host.docker.internal` 로 붙습니다.

**② 세 포트 전부 `127.0.0.1` 에만 바인딩합니다.**
앞단이 nginx이므로 직접 노출할 이유가 없습니다. **특히 Elasticsearch가 중요합니다** — 로컬 운영 편의를 위해 `xpack.security` 를 껐기 때문에, 외부에 열려 있으면 누구나 색인을 지울 수 있습니다.

**③ 비밀값은 `.env` 로 주입합니다.**
API 키(RAWG·TMDB)와 JWT 시크릿을 저장소에 두지 않습니다. `.gitignore` 에 포함돼 있습니다.

### 배포하며 밟은 것들

**★단일 파일 바인드 마운트는 inode를 묶습니다★**
nginx 설정을 `mv` 로 교체했더니 컨테이너가 **옛 inode를 계속 보면서** 문법 오류를 냈습니다.
파일 자체는 멀쩡했는데 말이죠. 바로 재시작했으면 같은 nginx를 쓰는 다른 서비스까지 내려갈 뻔했습니다.
→ 임시 컨테이너로 먼저 `nginx -t` 를 돌려 검증한 뒤 재시작.

**★서버가 아니라 테스트 도구가 실패하고 있었습니다★**
외부 접속이 안 돼 서버 문제로 의심했는데, Windows curl(schannel)이 TLS 핸드셰이크에서 깨진 것이었습니다.
`openssl s_client` 로는 인증서가 정상적으로 나왔습니다. **도구를 바꿔 재확인하지 않았으면 오진할 뻔했습니다.**

**국내 회선은 인바운드 80이 막혀 있습니다.**
443은 열리는데 80이 안 열립니다. HTTP→HTTPS 리다이렉트가 외부에서는 동작하지 않으므로 `https://` 로 접속해야 합니다.

**SSH 비대화형 셸에는 Docker 도구 경로가 없습니다.**
`docker-credential-desktop` 을 못 찾아 이미지 pull 이 실패합니다. `PATH` 를 명시해야 합니다.
